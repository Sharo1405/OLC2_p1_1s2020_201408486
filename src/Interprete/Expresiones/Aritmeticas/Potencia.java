 ...
 * @author sharolin
 */
public class Potencia extends Operacion implements Expresion {
            ...
            if ((tipo1.equals(Operacion.tipoDato.VECTOR) || tipo1.equals(Operacion.tipoDato.BOOLEAN)
                ...
                if (tipo1.equals(Operacion.tipoDato.VECTOR)) {
                    ...
                    if (tipoV1.equals(Operacion.tipoDato.ERRORSEMANTICO)) {
                        listas.errores.add(new NodoError(getLinea(), getColumna(), NodoError.tipoError.Semantico,
                                "El tipo de la Expreion 1 no es valida para realizar la POTENCIA"));
                ...
                if (normal.size() == 1 && normal2.size() == 1) {//--------------------------------------------------------------------
                    ...
                    switch (tipoResultante) {
                        case DECIMAL:
                            //Math.pow(base, n);
                            vectorParaDevolver.add(Math.pow(Double.parseDouble(String.valueOf(ob)), Double.parseDouble(String.valueOf(ob2))));
                            return vectorParaDevolver;

                        case ENTERO:
                            vectorParaDevolver.add(Math.pow(Integer.parseInt(String.valueOf(ob)), Integer.parseInt(String.valueOf(ob2))));
                            return vectorParaDevolver;

                        default:
                            listas.errores.add(new NodoError(getLinea(), getColumna(), NodoError.tipoError.Semantico,
                                    "Tipos no validos para realizar la POTENCIA Tipo Exp1: " + String.valueOf(tipo1)
                    ...
                    switch (tipoResultante) {
                        case DECIMAL:
                            for (int i = 0; i < normal.size(); i++) {
                                ...
                                item.add(Math.pow(Double.parseDouble(String.valueOf(ob)), Double.parseDouble(String.valueOf(ob2))));
                        ...
                        case ENTERO:
                            for (int i = 0; i < normal.size(); i++) {
                                Object ob = normal.get(i);
                                ArrayList<Object> item = new ArrayList<>();
                                item.add(Math.pow(Integer.parseInt(String.valueOf(ob)), Integer.parseInt(String.valueOf(ob2))));
                                vectorParaDevolver.add(item);
                            }
                            return vectorParaDevolver;
                ...
                listas.errores.add(new NodoError(this.getLinea(), this.getColumna(), NodoError.tipoError.Semantico, "Tipo de datos EXP1: "
                        + String.valueOf(tipo1) + " EXP2: " + String.valueOf(tipo2) + "No son validos para operarse como POTENCIA entre si"));
                return Operacion.tipoDato.ERRORSEMANTICO;
            }
        } catch (Exception e) {
    ...
    public Operacion.tipoDato getType(Entorno tablaDeSimbolos, ErrorImpresion listas) {
        ...
        try {
            ...
            if ((tipo1.equals(Operacion.tipoDato.VECTOR) || tipo1.equals(Operacion.tipoDato.BOOLEAN)
                ...
                Operacion.tipoDato ti = this.tipoResultante(tipo1, tipo2, tablaDeSimbolos, listas);
                if (ti.equals(Operacion.tipoDato.ENTERO)) {
                    ti = Operacion.tipoDato.DECIMAL;
                }
                return ti;
            ...
            System.out.println("Error en la clase POTENCIA en el getType()");
            return Operacion.tipoDato.ERRORSEMANTICO;
        }
    }
}
